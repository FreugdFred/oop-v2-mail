...
class LoadGooglePage:
    ...
    def __init__(self, KEYWORDS: str):
        ...
        self.URL = self.make_google_url()
        self.JSONCOOKIE = self.load_google_cookie()
        ...
        self.ALLHREFS = self.load_google_page()
        
    def return_hrefs(self) -> list:
        ''' print all hrefs in ALLHREFS '''
        return self.ALLHREFS
    
    def make_google_url(self) -> str:
    ...
    def scroll_to_href(self, page: object, href: str) -> None:
        ''' scroll to href into view if needed with playwright '''
        page.wait_for_timeout(1000)
        element = page.locator(f'[href="{href}"]').nth(-1)
        element.scroll_into_view_if_needed()  
        
    def stop_scroll_bool(self, start_time: float, html_page: str) -> bool:
    ...
    def load_google_page(self) -> list:
        ...
        with sync_playwright() as plays:
            ...
            while not self.stop_scroll_bool(start_time, html_page):
                html_page = page.content()
                allhrefsList = list(self.get_hrefs_from_html_page(html_page))
                last_href = allhrefsList[-1]
                self.scroll_to_href(page, last_href)
